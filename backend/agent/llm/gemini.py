...
Thinking policy:
  ...
  - On gemini-2.x models, ``thinking_budget`` is clamped to 1024 tokens (minimum
    meaningful budget) unless the caller overrides via ``thinking_budget`` kwarg.
    ...
    async def complete(
        ...
        try:
            ...
            if "gemini-3" in self.model.lower():
                ...
                # gemini-2.5+: set minimum thinking budget to keep latency low
