        ...
        with sqlite3.connect(str(memory.db_file), timeout=5) as conn:
            ...
            if count == 0:
                conn.execute(
                    ...
                    ("0 8 * * *", "Analyse last night's sleep quality and morning readiness. Include heart rate, HRV, and recovery metrics.", "active"),
                )
                conn.commit()
                logger.info("Inserted default scheduled task (daily 08:00 sleep analysis)")
    except Exception as e:
